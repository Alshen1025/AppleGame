...
public:
	...
	virtual void TextRender(HDC hdc) = 0;
	...
	void ChaseTarget(Vector2f target, Vector2f self);
...
public:
	...
	void TextRender(HDC hdc) override;

	void SetColliderCircle(float radius);
	void SetColliderBox(float halfWidth, float halfHeight);

	void SetAppleNumber(int number) { AppleNumber = number; }
	int GetAppleNumber() { return AppleNumber; }

	void SetIsDelete(bool state) { isDelete = state; }
	bool GetIsDelete() { return isDelete; }

protected:
	void DrawCollider(HDC hdc);
	void DrawAppleText(HDC hdc);
	void Move(float deltaTime);
